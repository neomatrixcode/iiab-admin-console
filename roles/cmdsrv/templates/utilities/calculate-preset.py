...
presets_dir = '/opt/iiab/iiab-admin-console/roles/cmdsrv/files/presets/'
               #adm.CONST.admin_install_base + '/cmdsrv/files/presets/'
role_stats = {}
content = {}
content["zims"] = []
content["modules"] = []
...
def main():
    ...
    this_preset_dir = presets_dir + preset + '/'
    print(this_preset_dir)
    if not os.path.exists(this_preset_dir):
        #os.makedirs(this_preset_dir)
        print("the preset not exits")

    role_stats = adm.get_roles_status()

    content= content_from_menu(this_preset_dir)
...
def content_from_menu(this_preset_dir):
    ...
    for menu_def in menu["menu_items_1"]:
        if menu_def in all_menu_defs:
            try:
                print(all_menu_defs[menu_def]["footnote"])
                #print(all_menu_defs[menu_def]['size'])
            except:
                continue
